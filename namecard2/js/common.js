            ...
            getOrientation(file, function(orientation) {
                ...
                if(orientation==-2||orientation==-1||orientation==1){
                   console.log('orientation',orientation)
                    ...
                    var lxcanvas = document.createElement('canvas');
                    lxcanvas.style.backgroundColor = '#fff';
                    lxcanvas.style.display = 'none';
                    lxcanvas.width= upimg.width;
                    lxcanvas.height = c1.height;
                    var $body = $("body");
                    $body.append(lxcanvas);
                    var lxctx = lxcanvas.getContext('2d');
                    lxctx.clearRect(0,0,lxcanvas.width,lxcanvas.height);
                    lxctx.save();
                    //lxctx.translate(lxcanvas.width/2/ratio,lxcanvas.height/2/ratio);//设置画布上的(0,0)位置，也就是旋转的中心点
                    lxctx.translate(0,0);
                    lxctx.rotate(90*Math.PI/180);
                    lxctx.drawImage(upimg,0,-lxcanvas.height ,lxcanvas.width,lxcanvas.height);
                    lxctx.restore();
                    ctx1.drawImage(lxcanvas,0,0,upimg.width,c1.height);
                }
